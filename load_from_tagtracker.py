...
import modules.tt_datafile as tt_datafile
import modules.tt_dbutil as tt_dbutil
...
from modules.tt_event import Event
...
class DayStats:
    ...
    max_regular_time: BTime = ""
    max_oversize_num: int = 0
    max_oversize_time: BTime = ""
    max_total_num: int = 0
    max_total_time: BTime = ""
    time_open: BTime = ""
    time_close: BTime = ""
    weekday: int = None
    registrations: int = 0
...
def calc_stay_length(hhmm_in: str, hhmm_out: str) -> str:
    ...
    t_in = BTime(hhmm_in).num
    t_out = BTime(hhmm_out).num
    t_stay = t_out - t_in
    hhmm_stay = BTime(t_stay)

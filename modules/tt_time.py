"""BTime str object represents time as HH:MM.
...
Invalid input results in a blank BTime object.

Copyright (c) 2024 Todd Glover
Based on github.com/ironwoodcall/tagtracker (C) 2023-2024 Julias Hocking & Todd Glover
...
class BTime(str):
    ...
    @staticmethod
    def _time_int(maybe_time: str,allow_gt_24:bool=True) -> int:
        """Convert known-good string representation of time to int (or None).

        If allow_gt_24 is False then does not allow times > 24 hours.
        ...
        if h < 0 or (h > 24 and not allow_gt_24) or not (0 <= m <= 59):
            return None
        return as_int

    @staticmethod
    def _find_time(maybe_time, allow_gt_24:bool=True) -> str:
        ...
        if isinstance(maybe_time, int):
            if 0 <= maybe_time <= 60 * 24 or allow_gt_24:
                h = maybe_time // 60
                m = maybe_time % 60
        ...
        if maybe_time.strip().lower() == "now":
            ...
            return (timenow, BTime._time_int(timenow))
        # Candidate might be some kind of HHMM
        as_int = BTime._time_int(maybe_time)
        if as_int is None:
            return ("", None)
        return BTime._find_time(as_int)

    def __new__(cls, maybe_time=""):
        """Create a BTime string with its 'self' as canonical time."""
    ...
    def __eq__(self, other) -> bool:
        ...
        return bool(str(self) == str(BTime(other)))
